...
int main(int argc,char **argv)
{
    ...
    if(rank==0)
    {
      ...
      {
        //RING NETWORK
        MPI_Isend(indexes,block,MPI_INT,(rank+1)%processes,rank,MPI_COMM_WORLD,&request[0]);
        MPI_Isend(data,block*dimensions,MPI_DOUBLE,(rank+1)%processes,rank,MPI_COMM_WORLD,&request[1]);

        MPI_Irecv(indexes_temp,block,MPI_INT,processes-1,processes-1,MPI_COMM_WORLD,&request[2]);
        MPI_Irecv(data_received,block*dimensions,MPI_DOUBLE,processes-1,processes-1,MPI_COMM_WORLD,&request[3]);
        ...
        {
          MPI_Isend(indexes_temp,block,MPI_INT,(rank+1)%processes,rank,MPI_COMM_WORLD,&request[0]);
          MPI_Isend(data_received,block*dimensions,MPI_DOUBLE,(rank+1)%processes,rank,MPI_COMM_WORLD,&request[1]);

          MPI_Irecv(indexes,block,MPI_INT,processes-1,processes-1,MPI_COMM_WORLD,&request[2]);
          MPI_Irecv(data_received_2,block*dimensions,MPI_DOUBLE,processes-1,processes-1,MPI_COMM_WORLD,&request[3]);
        ...
        time_end = MPI_Wtime();
        printf("KNN MPI NON BLOCKING time = %f\n", time_end-time_start);
        printf("KNN MPI NON BLOCKING calculation time = %f\n",calculation_time);
        printf("KNN MPI NON BLOCKING communication time = %f\n",time_end-time_start-calculation_time);
        ...
        FILE *output=fopen("result_mpi_non_blocking.bin","wb");
        if(output==NULL)
        {
          printf("Error opening the file : result_mpi_non_blocking.bin\n");
          exit(1);
        }
        for(int i=0;i<block*k_neighbours;i++)
        {
          ...
          if(return_value!=1)
          {
            printf("Error writing to file : result_mpi_non_blocking.bin\n");
            exit(1);
          }
        }
    ...
    {
      //RING NETWORK
      MPI_Irecv(indexes_temp,block,MPI_INT,rank-1,rank-1,MPI_COMM_WORLD,&request[0]);
      MPI_Irecv(data_received,block*dimensions,MPI_DOUBLE,rank-1,rank-1,MPI_COMM_WORLD,&request[1]);

      MPI_Isend(indexes,block,MPI_INT,(rank+1)%processes,rank,MPI_COMM_WORLD,&request[2]);
      MPI_Isend(data,block*dimensions,MPI_DOUBLE,(rank+1)%processes,rank,MPI_COMM_WORLD,&request[3]);

      KNN_MPI(block,rank);
      MPI_Waitall(4,request,status);
      for(int p=0;p<processes-1;p++)
      {
        MPI_Irecv(indexes,block,MPI_INT,rank-1,rank-1,MPI_COMM_WORLD,&request[0]);
        MPI_Irecv(data_received_2,block*dimensions,MPI_DOUBLE,rank-1,rank-1,MPI_COMM_WORLD,&request[1]);

        MPI_Isend(indexes_temp,block,MPI_INT,(rank+1)%processes,rank,MPI_COMM_WORLD,&request[2]);
        MPI_Isend(data_received,block*dimensions,MPI_DOUBLE,(rank+1)%processes,rank,MPI_COMM_WORLD,&request[3]);
...
void test_comparing_with_matlab(void)
{
  ...
  input_c=fopen("result_mpi_non_blocking.bin","rb");
  if(input_c==NULL)
  {
    printf("error opening the file : result_mpi_non_blocking.bin\n");
    exit(1);
  ...
  {
    printf("result_mpi_non_blocking.bin and result_matlab.bin Do not have the same size\n");
    exit(1);
  }
  rewind(input_c);
  rewind(input_matlab);
...
void test_with_sequential(void)
{
  ...
  input_mpi=fopen("result_mpi_non_blocking.bin","rb");
  if(input_mpi==NULL)
  {
    printf("error opening the file : result_mpi_non_blocking.bin\n");
    exit(1);
  ...
  {
    printf("result_mpi_non_blocking.bin and result_sequential.bin Do not have the same size\n");
    exit(1);
  }
  rewind(input_mpi);
